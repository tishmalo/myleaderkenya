...
class AspirantToolController extends Controller
{
    ...
    public function importSupportContacts(Request $request): RedirectResponse
    {
        $candidate = $this->workspaceService->candidateForUser($request->user());
        ...
        $validated = $request->validate([
            'support_group_type_id' => ['nullable', 'exists:support_group_types,id'],
            'contacts_file' => ['required', 'file', 'max:5120', 'mimes:xlsx,xls,csv,txt'],
        ]);

        $defaultGroupId = $validated['support_group_type_id'] ?? null;
        if ($defaultGroupId && ! SupportGroupType::active()->whereKey($defaultGroupId)->exists()) {
            throw \Illuminate\Validation\ValidationException::withMessages([
                'support_group_type_id' => 'Choose an active default support group.',
            ]);
        }

        $rows = $this->supportContactImportRows($request->file('contacts_file'));
        $groupTypes = SupportGroupType::active()->ordered()->get();
        $groupLookup = $groupTypes->flatMap(fn (SupportGroupType $type) => [
        ...
        DB::transaction(function () use ($rows, $candidate, $request, $defaultGroupId, $groupLookup, &$imported, &$skipped, &$errors): void {
        ...
        $message = 'Imported ' . number_format($imported) . ' support contacts.';
        if ($skipped > 0) {
            $message .= ' Skipped ' . number_format($skipped) . ' rows. ' . implode(' ', $errors);
        }

        return redirect()->route('aspirant.tools.show', 'support-groups')->with($imported > 0 ? 'success' : 'warning', $message);
    }
    ...
    private function supportContactImportRows(\Illuminate\Http\UploadedFile $file): array
    {
        $extension = Str::lower($file->getClientOriginalExtension());

        if (in_array($extension, ['csv', 'txt'], true)) {
            return $this->csvSupportContactRows($file->getRealPath());
        }

        if ($extension === 'xlsx') {
            return $this->xlsxSupportContactRows($file->getRealPath());
        }

        if ($extension === 'xls') {
            throw \Illuminate\Validation\ValidationException::withMessages([
                'contacts_file' => 'Legacy XLS files are not supported. Save the file as XLSX or CSV, then upload again.',
            ]);
        }
