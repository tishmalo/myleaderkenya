...
<div class="max-w-7xl mx-auto">
    <div class="flex justify-between items-center mb-8">
        <h1 class="text-3xl font-semibold text-white flex items-center gap-3">
            <i class="fas fa-map"></i> Counties
        </h1>
        <div class="flex gap-3">
            <a href="{{ route('counties.create') }}" class="bg-emerald-600 hover:bg-emerald-700 px-6 py-3 rounded-2xl font-medium flex items-center gap-2">
            ...
            <label class="cursor-pointer bg-zinc-700 hover:bg-zinc-600 px-6 py-3 rounded-2xl font-medium flex items-center gap-2">
                ...
                <input type="file" id="countyImport" accept=".json" class="hidden" onchange="importCounties(this)">
            </label>
        </div>
    </div>
    ...
    <div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 mb-8">
        <form method="GET" id="searchForm" class="flex gap-4">
            ...
            <a href="{{ route('counties.index') }}" class="bg-zinc-800 hover:bg-zinc-700 px-8 rounded-2xl font-medium flex items-center">Clear</a>
        </form>
    </div>
        ...
        <table class="w-full">
            <thead class="bg-zinc-950">
                <tr class="border-b border-zinc-800">
                    ...
                    <th class="px-8 py-5 text-center">Capital</th>
            ...
            <tbody class="divide-y divide-zinc-800">
                ...
                <tr class="hover:bg-zinc-800/70 transition-colors">
                    ...
                    <td class="px-8 py-6 text-center">
                        <button onclick="showPollingStations({{ $county->id }}, 'county')" 
                                class="text-emerald-400 hover:text-emerald-500 font-medium underline">
                            {{ $county->polling_stations_count ?? 0 }}
                        </button>
                    </td>
                    <td class="px-8 py-6 text-center font-medium text-emerald-400">
                        {{ number_format($county->registered_voters ?? 0) }}
                    </td>

                    <td class="px-8 py-6 text-center">{{ $county->capital ?? '—' }}</td>
                    <td class="px-8 py-6 text-center">
                        <a href="{{ route('counties.edit', $county) }}" class="text-emerald-400 hover:text-emerald-500 mr-4">Edit</a>
                        <button onclick="showDeleteModal('{{ route('counties.destroy', $county) }}', 'Delete this county?')" 
                                class="text-red-400 hover:text-red-500">Delete</button>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="7" class="px-8 py-20 text-center text-zinc-500">No counties found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
function showPollingStations(id, type) {
    ...
    content.innerHTML = `<div class="text-center py-12"><i class="fas fa-spinner fa-spin text-4xl text-emerald-500"></i><p class="mt-4 text-zinc-400">Loading polling stations...</p></div>`;
    ...
    fetch(`/api/polling-stations/${type}/${id}`)
                ...
                <table class="w-full">
                    <thead class="bg-zinc-950 sticky top-0">
                        <tr class="border-b border-zinc-700">
                            <th class="px-6 py-4 text-left">Office / Station</th>
                ...
                data.forEach(station => {
                    html += `
                        <tr class="hover:bg-zinc-800">
                            ...
                            <td class="px-6 py-4">${station.ward || '—'}</td>
        ...
        .catch(err => {
            content.innerHTML = `<p class="text-red-400 text-center py-8">Failed to load data. Please try again.</p>`;
            console.error(err);
        });
}
