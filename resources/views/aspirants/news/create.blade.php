...
@section('title', 'Submit Campaign News - My Leader Kenya')
@push('styles')
...
<p class="news-kicker">Aspirant Dashboard</p><h1 class="news-title">Submit campaign news</h1><p class="news-subtitle">{{ $candidate->name }} will be tagged automatically. The article remains private until administrator approval.</p>
<div class="news-actions"><a class="news-button" href="{{ route('aspirant.news.index') }}">&larr; Campaign News</a></div>
...
<form class="news-form" method="POST" action="{{ route('aspirant.news.store') }}" enctype="multipart/form-data">@csrf
...
<div class="news-field full"><x-remote-multiselect name="candidates[]" label="Tagged Aspirants" :search-url="route('aspirant.news.candidates.search')" :selected="$selectedCandidates" placeholder="Search aspirants by name or nickname..." empty-text="No approved aspirants match your search." /><p class="news-help">The active aspirant is required and automatically restored. You may tag other aspirants discussed in the article.</p>@error('candidates')<p class="news-error">{{ $message }}</p>@enderror</div>
