...
<p class="news-kicker">Aspirant Dashboard</p><h1 class="news-title">Submit an event</h1><p class="news-subtitle">Your event stays private until an administrator approves it. Set your own ticket price — enter 0 for a free event.</p>
<div class="news-actions"><a class="news-button" href="{{ route('aspirant.events.index') }}">&larr; My Events</a></div>
...
<form class="news-form" method="POST" action="{{ route('aspirant.events.store') }}" enctype="multipart/form-data">@csrf
...
<div class="news-field"><label for="price">Ticket price (KES) *</label><input id="price" type="number" name="price" value="{{ old('price', 0) }}" min="0" max="99999999" step="0.01" required placeholder="0.00"><p class="news-help">Enter 0 for a free event.</p>@error('price')<p class="news-error">{{ $message }}</p>@enderror</div>
