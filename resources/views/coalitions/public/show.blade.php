...
@section('title', ($coalition->meta_title ?: $coalition->name) . ' - Coalition')
@section('meta_description', $coalition->meta_description ?: ($coalition->excerpt ?: Str::limit(strip_tags($coalition->content), 155)))
@section('og_image', $coalition->logo ? Storage::url($coalition->logo) : asset('images/myleader.png'))
...
<div class="party-back"><a href="{{ route('coalitions.public') }}"><i class="fas fa-arrow-left"></i> All Coalitions</a></div>
<main class="party-show">
    <article class="party-show-card">
        <header class="party-show-head {{ $coalition->logo ? 'has-logo' : '' }}" style="background:linear-gradient(135deg, {{ $coalition->brand_color ?: '#BB0000' }}33, rgba(0,102,0,0.16));">
            @if($coalition->logo)<div class="party-show-logo"><img src="{{ Storage::url($coalition->logo) }}" alt="{{ $coalition->name }}"></div>@endif
            <div><div class="party-show-kicker">Political Coalition</div><h1 class="party-show-title">{{ $coalition->name }}</h1>@if($coalition->excerpt)<p class="party-show-excerpt">{{ $coalition->excerpt }}</p>@endif</div>
        </header>
        <div class="party-content">{!! nl2br(e($coalition->content)) !!}</div>
        <section class="member-section"><h2>Member Political Parties</h2><x-member-party-list :parties="$coalition->politicalParties" /></section>
    </article>
</main>
@endsection
