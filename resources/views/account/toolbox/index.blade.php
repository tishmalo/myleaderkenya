...
@forelse($directSupports as $support)<article class="toolbox-row"><div><h3 class="font-bold text-white">{{ $support->candidate->name ?? 'Aspirant' }}</h3><p class="toolbox-muted">{{ $support->currency }} {{ number_format($support->gross_amount, 2) }} &middot; Sent directly to the aspirant, less applicable transaction and platform fees</p><p class="mt-2 text-white">{{ $support->message }}</p>@if($support->aspirant_reply)<div class="toolbox-reply"><strong>Reply from the aspirant</strong><br>{{ $support->aspirant_reply }}</div>@endif</div><span class="toolbox-status {{ $support->status }}">{{ ucfirst($support->status) }}</span></article>@empty<div class="toolbox-card toolbox-muted">You have not sent direct support to an aspirant yet.</div>@endforelse
</div></section>
