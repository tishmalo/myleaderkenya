...
<section class="toolbox-section"><h2 class="news-title" style="font-size:30px">Top up your Toolbox</h2><p class="toolbox-muted">Purchased tokens stay in your personal wallet until you choose a sponsorship.</p><div class="toolbox-grid">
@forelse($packages as $package)<article class="toolbox-card"><h3 class="text-xl font-bold">{{ $package->name }}</h3><p class="mt-2 text-3xl font-bold text-emerald-300">{{ number_format($package->token_amount) }} tokens</p><p class="toolbox-muted">{{ $package->currency }} {{ number_format($package->price, 2) }}</p><form method="POST" action="{{ route('account.toolbox.purchase') }}" class="toolbox-form">@csrf<input type="hidden" name="candidate_token_package_id" value="{{ $package->id }}"><input class="toolbox-input" name="phone" value="{{ old('phone', auth()->user()->phone) }}" placeholder="Payment phone" required><input class="toolbox-input" type="email" name="email" value="{{ old('email', auth()->user()->email) }}" placeholder="Receipt email" required><button class="toolbox-button"><i class="fas fa-wallet mr-2"></i> Buy tokens</button></form></article>
@empty<div class="toolbox-card toolbox-muted">No token packages are available right now.</div>@endforelse
</div></section>
<section class="toolbox-section"><h2 class="news-title" style="font-size:30px">My sponsorship requests</h2><div class="toolbox-list">
...
<section class="toolbox-section"><h2 class="news-title" style="font-size:30px">Transaction history</h2><div class="toolbox-list">@forelse($transactions as $transaction)<article class="toolbox-row"><div><h3 class="font-bold text-white">{{ $transaction->action_label }}</h3><p class="toolbox-muted">{{ $transaction->candidate->name ?? 'Personal Toolbox' }} - {{ $transaction->created_at->format('d M Y, H:i') }}</p></div><strong class="toolbox-amount {{ $transaction->amount >= 0 ? 'in' : 'out' }}">{{ $transaction->amount >= 0 ? '+' : '' }}{{ number_format($transaction->amount) }} tokens</strong></article>@empty<div class="toolbox-card toolbox-muted">No Toolbox transactions yet.</div>@endforelse</div></section>
<section class="toolbox-section"><h2 class="news-title" style="font-size:30px">Purchase history</h2><div class="toolbox-list">@forelse($purchases as $purchase)<article class="toolbox-row"><div><h3 class="font-bold text-white">{{ $purchase->package_name }}</h3><p class="toolbox-muted">{{ $purchase->currency }} {{ number_format($purchase->price,2) }} - {{ $purchase->created_at->format('d M Y, H:i') }}</p></div><span class="toolbox-status {{ $purchase->status }}">{{ ucfirst($purchase->status) }}</span></article>@empty<div class="toolbox-card toolbox-muted">No token purchases yet.</div>@endforelse</div></section>
</section></div></main>
@endsection
