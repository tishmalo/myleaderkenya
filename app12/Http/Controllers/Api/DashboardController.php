...
public function getConstituenciesByCounty(Request $request, $county)
{
...
public function getConstituenciesByCountyApi(Request $request, $county){
    
 
    $constituencies = \App\Models\Constituency::where('county_id', function($query) use ($county) {
                        $query->select('id')
                              ->from('counties')
                              ->where('name', $county)
                              ->limit(1);
